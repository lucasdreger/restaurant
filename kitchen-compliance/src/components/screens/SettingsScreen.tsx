...
export function SettingsScreen({ onBack }: SettingsScreenProps) {
  ...
  useEffect(() => {
    if (activeTab === 'staff' && currentSite?.id && !dataLoaded.staff) {
      loadStaff()
    } else if (activeTab === 'food' && (currentSite?.id || !dataLoaded.food)) {
      // Allow loading global food even without site_id, but reload if site_id appears
      if (!dataLoaded.food) loadFood()
    } else if (activeTab === 'fridges' && currentSite?.id && !dataLoaded.fridges) {
      loadFridges()
    }
  }, [activeTab, currentSite?.id, dataLoaded.staff, dataLoaded.food, dataLoaded.fridges])

  // Suggested Staff Code logic
  useEffect(() => {
    if (staffList.length > 0 && !newStaffName && !newStaffCode) {
      const numericCodes = staffList
        .map(s => parseInt(s.staff_code || '0', 10))
        .filter(n => !isNaN(n) && n > 0)

      const nextCode = numericCodes.length > 0 ? Math.max(...numericCodes) + 1 : 1
      setNewStaffCode(nextCode.toString())
    } else if (staffList.length === 0 && !newStaffName && !newStaffCode) {
      setNewStaffCode('1')
    }
  }, [staffList, newStaffName, newStaffCode])

  // Clear local lists when site changes to force a fresh view
  useEffect(() => {
    if (currentSite?.id) {
      setStaffList([])
      setFoodList([])
  ...
  // Load site settings for API keys and preferences
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        updateSettings({
          theme: (settingsRecord.theme as AppTheme) || settings.theme,
  ...
  const persistSiteSettings = async (updates: Partial<typeof settings>) => {
    ...
    try {
      await upsertSiteSettings({
        ...
        audio_model: updates.audioModel ?? settings.audioModel,
        openai_api_key: updates.openaiApiKey ?? settings.openaiApiKey,
        openrouter_api_key: updates.openrouterApiKey ?? settings.openrouterApiKey,
          ...
          {activeTab === 'staff' && (
            <div className="space-y-6 animate-slide-in">
              <div className="card-stunning p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end bg-theme-ghost p-4 rounded-xl border border-theme-primary/50">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold uppercase text-theme-muted">Staff Code</label>
                    <input
                      value={newStaffCode}
              ...
              <div className="card-stunning p-6">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ voiceProvider: 'browser' })
                        persistSiteSettings({ voiceProvider: 'browser' })
                      }}
                      className={cn(
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ voiceProvider: 'openai' })
                        persistSiteSettings({ voiceProvider: 'openai' })
                      }}
                      className={cn(
                        "p-4 rounded-xl border-2 transition-all text-left",
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ voiceProvider: 'openrouter' })
                        persistSiteSettings({ voiceProvider: 'openrouter' })
                      }}
                      className={cn(
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {([
                        ...
                        <button
                          key={model.id}
                          onClick={() => {
                            updateSettings({ audioModel: model.id })
                            persistSiteSettings({ audioModel: model.id })
                          }}
                          className={cn(
                  ...
                  <div className={cn(
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        persistSiteSettings({ openaiApiKey: value })
                      }}
                      placeholder="sk-..."
                  ...
                  <div className={cn(
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        persistSiteSettings({ openrouterApiKey: value })
                      }}
                      placeholder="sk-or-..."
                      className="input-stunning font-mono text-sm"
                    />
              ...
              <div className="card-stunning p-6">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ ocrProvider: 'tesseract', ocrModel: 'tesseract' })
                        persistSiteSettings({ ocrProvider: 'tesseract', ocrModel: 'tesseract' })
                      }}
                      className={cn(
                        "p-4 rounded-xl border-2 transition-all text-left",
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ ocrProvider: 'openai', ocrModel: 'openai/gpt-4o' })
                        persistSiteSettings({ ocrProvider: 'openai', ocrModel: 'openai/gpt-4o' })
                      }}
                      className={cn(
                        "p-4 rounded-xl border-2 transition-all text-left",
                    ...
                    <button
                      onClick={() => {
                        updateSettings({ ocrProvider: 'openrouter', ocrModel: 'google/gemini-2.0-flash' })
                        persistSiteSettings({ ocrProvider: 'openrouter', ocrModel: 'google/gemini-2.0-flash' })
                      }}
                      className={cn(
                        "p-4 rounded-xl border-2 transition-all text-left",
